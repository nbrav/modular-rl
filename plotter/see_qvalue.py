...
if PLOT_QVALUE:
    ...
    im1 = plt.imshow(Q[0,0,:,:,1],interpolation="none",cmap=plt.cm.orange)
...
if PLOT_QVALUE_ALL:
    ...
    #cb = fig.colorbar(mpl.cm.Oranges);
    ...
    for state_i in state_file:
        ...
        if t%_updstep==0:            
            ...
            colors = Qtemp
            p = PatchCollection(patches, cmap=mpl.cm.Oranges, alpha=1)
            ...
            if not COLORBAR:
                ...
                cb.update_bruteforce(p)
           
            Qtemp = Q[t,1,:,:,:];
            Qtemp = np.reshape(Qtemp,(500))
            colors = Qtemp 
            p = PatchCollection(patches, cmap=mpl.cm.Blues, alpha=1)
            p.set_array(np.array(colors))
            p.set_clim(Qmin,Qmax)

            ax2.add_collection(p)
            #ax2.set_title('Qmax:'+str(round(np.max(Q[:,1,:,:,:]),1))+' Qmin:'+str(round(np.min(Q[:,1,:,:,:]),2)))
...
savefig('qvalue.png', bbox_inches='tight')
